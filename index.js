...
client.on('message', msg => {
  ...
  const {get} = require("snekfetch");
	...
	if (!userMention) {
		...
		.addField(`Roles`, `WIP`)
		.addField(`Joined guild`, `${msg.member.joinedAt.substr(0, 15)}`)
		.addField(`Joined Discord`, `${msg.member.createdAt.substr(0, 15)}`)
		...
		.addField(`Roles`, `WIP`)
		.addField(`Joined guild`, `${userMention.joinedAt.substr(0, 15)}`)
		.addField(`Joined Discord`, `${userMention.createdAt.substr(0, 15)}`)
